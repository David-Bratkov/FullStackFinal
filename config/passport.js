...
passport.use(
   new GoogleStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
         ...
         if (!user) {
            // console.log("DEBUG: ", profile.emails[0].value);
            let localuser = await User.findOne({ email: profile.emails[0].value });
            if (localuser) {
               // If the user exists but has a local account, update their Google ID
               localuser.googleId = profile.id; // Set the Google ID
               localuser.name = profile.displayName; // Update the name
               localuser.picture = profile.photos[0].value; // Update the picture
               await localuser.save(); // Save the updated user object
               user = localuser; // Use the updated user object
